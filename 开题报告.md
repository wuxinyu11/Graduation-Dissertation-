...
(1)基于Hellinger-Reissner原理的变分一致型伽辽金无网格法 ，首先Hellinger-Ressiner原理是采用余能泛函进行求解弱形式，将弱形式分为位移和应力两部分，其次位移采用再生核近似离散的方式，应力采用再生光滑tidy近似离散，采用混合离散得出内嵌再生光滑梯度表达式的变分原理，最后，通过分片实验验证基于Hellinger-Ressiner变分原理的本质边界条件方法，再生光滑梯度积分法满足变分一致性。

(2)变分一致型伽辽金无网格法背景积分域优化划分方案 首先，将无网格节点和背景积分域分开，数值计算时同一套无网格节点，不同的背景积分域，其次，根据简单的一维和二维模型，得出背景积分域和无网格节点大致关系，最后通过典型的数值算例施加优化后的积分域方案，提高计算效率。

(3)具有变分一致性的无网格本质边界条件施加方案 首先选择高斯-Nitsche法，高斯-罚函数法；RKGSI-Nitsche法;RKGSI-罚函数法；RKGSI-HR法不同的施加方法进行对比，其次通过数值算例分别施加不同的施加方法得出$L_2$误差和$H_1$误差，最后根据误差进行对比不同施加方法的精度和计算效率。
...
| 时间            | 研究内容                                                     | 进度情况  |
| --------------- | ------------------------------------------------------------ | --------- |
| 2022.7-2022.9   | 分析和对比现有传统无网格解析方法的优缺点，提出基于Hellinger-Reissner原理的变分一致型伽辽金无网格法 | $\bullet$ |
| 2022.10-2022.11 | 在传统弹性力学问题已经验证Hellinger-Ressiner变分原理施加本质边界条件方法的变分一致性，计算精度和效率。 | $\bullet$ |
| 2022.12-2023.6  | 将满足变分一致性的Hellinger-Ressiner变分原理引入薄板问题变形分析中，进一步通过数值算例进行验证该变分原理的的精度和效率。 | $\circ$   |
| 2023.7-2023.12  | 针对变分一致型伽辽金无网格打的背景积分域进行优化划分方案，得出结论，提高计算效率 | $\circ$   |
